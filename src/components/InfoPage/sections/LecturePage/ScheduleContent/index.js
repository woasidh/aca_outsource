...
function ScheduleContent(props) {
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:3001/schedule/next?id=${id}`).then(res => {
            if (res.data.success !== true) {
                alert('오류 발생');
        ...
        axios.get(`http://localhost:3001/schedule/pre?id=${id}`).then(res => {
            if (res.data.success !== true) {
                alert('오류 발생');
                console.log(res.data.err);
                /* window.location.href = '/' */
    ...
    return (
        <div style={{ width: '100%' }}>
            ...
            <a href="/info/community/schedule" className="noticeCnt__listBox">
            ...
            {isNext ? <div className="noticeCnt__Box" id="pre">
                ...
                <div className="noticeCnt__Box__right" d>
                    <span><a href = {'/info/community/schedule/content?id='+ nextId}>{nextTitle}</a></span>
                </div>
            </div> : <></>}
            {isPre ? <div className="noticeCnt__Box" id="pre">
                ...
                <div className="noticeCnt__Box__right" d>
                    <span><a href = {'/info/community/schedule/content?id='+ preId}>{preTitle}</a></span>
                </div>
            </div> : <></>}
        </div>
    )
