...
function NoticeContent(props) {
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:3001/video/next?id=${id}`).then(res => {
            if (res.data.success !== true) {
                alert('오류 발생');
                window.location.href = '/'
            } else {
        ...
        axios.get(`http://localhost:3001/video/pre?id=${id}`).then(res => {
            if (res.data.success !== true) {
                alert('오류 발생');
                window.location.href = '/'
            } else {
    ...
    return (
        <div style={{ width: '100%' }}>
            ...
            <a href="/info/community/video" className="noticeCnt__listBox">
