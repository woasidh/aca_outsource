...
function UnivContent(props) {
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:3001/univInfo/next?id=${id}`).then(res => {
            if (res.data.success !== true) {
                alert('오류 발생');
                window.location.href = '/'
            } else {
        ...
        axios.get(`http://localhost:3001/univInfo/pre?id=${id}`).then(res => {
            if (res.data.success !== true) {
                alert('오류 발생');
                window.location.href = '/'
            } else {
    ...
    return (
        <div style={{ width: '100%' }}>
            <p className="noticeCnt__main">입시정보</p>
            ...
            <a href="/info/community/univInfo" className="noticeCnt__listBox">
            ...
            {isNext ? <div className="noticeCnt__Box" id="pre">
                ...
                <div className="noticeCnt__Box__right" d>
                    <span><a href = {'/info/community/univInfo/content?id='+ nextId}>{nextTitle}</a></span>
                </div>
            </div> : <></>}
            {isPre ? <div className="noticeCnt__Box" id="pre">
                ...
                <div className="noticeCnt__Box__right" d>
                    <span><a href = {'/info/community/univInfo/content?id='+ preId}>{preTitle}</a></span>
                </div>
            </div> : <></>}
        </div>
    )
